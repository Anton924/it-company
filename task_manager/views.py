...
class TeamListView(
    ...
    def get_queryset(self):
        ...
        queryset = queryset.select_related(
            "team_lead"
        ).prefetch_related("workers", "projects")
...
class WorkerListView(
    ...
    def get_queryset(self):
        ...
        if self.form.is_valid():
            ...
            queryset = queryset.filter(
                Q(
                    Q(first_name__icontains=val_1),
                    Q(last_name__icontains=val_2)
                ) | Q(
                    Q(first_name__icontains=val_2),
                    Q(last_name__icontains=val_1)
                )
            )

        queryset = queryset.prefetch_related(
            "teams_team_lead", "teams", "tasks"
...
class WorkerDetailView(
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["undone_tasks"] = Task.objects.filter(
            is_completed=False, assignees=self.object.pk
        ).order_by("deadline")
        context["done_tasks"] = Task.objects.filter(
            is_completed=True, assignees=self.object.pk
        ).order_by("deadline")
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        queryset = queryset.prefetch_related(
            "teams__workers", "teams__team_lead"
        ).select_related("position")
...
class ProjectListView(
    ...
    def get_queryset(self):
        ...
        if self.form.is_valid():
            queryset = queryset.filter(
                name__icontains=self.form.cleaned_data["name"]
            )
